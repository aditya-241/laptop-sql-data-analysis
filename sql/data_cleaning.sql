...
UPDATE laptops  
SET cpu_speed = CAST(REPLACE(SUBSTRING_INDEX(Cpu,' ',-1),'GHz','') 
AS DECIMAL(10,2));
...
SET cpu_name = TRIM(
    SUBSTRING_INDEX(
        ...
        ' ',
        (LENGTH(REPLACE(Cpu, cpu_brand, '')) - LENGTH(REPLACE(REPLACE(Cpu, cpu_brand, ''), ' ', ''))) / LENGTH(' ')
    )
);
...
UPDATE laptops
SET 
  resolution_width = CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(ScreenResolution, ' ', -1), 'x', 1) AS UNSIGNED),
  resolution_height = CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(ScreenResolution, ' ', -1), 'x', -1) AS UNSIGNED);
